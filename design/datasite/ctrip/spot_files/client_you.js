...
var sso_Config = {
    url: {
        verifyCodeUrl: sso_consts.rootUrl + '/member/ajax/VerifyCodeHandler.ashx',
        verifyCodePhoneUrl: sso_consts.rootUrl + '/member/ajax/VerifyCodePhoneHandler.ashx'
    },
    jsonpUrl: {
        checkIsBlack: sso_consts.rootUrl + '/member/ajax/AjaxCheckVerifyCodeAndIP.aspx?jsonp=JSP.checkIsBlack', //返回值： N：不需要验证码，T:需要验证码
        ...
        simpleValidateCode: sso_consts.rootUrl + '/member/ajax/SimpleValidateCode.aspx',
    ...
    thirdUrl: {
        ...
        pophao360: sso_consts.rootUrl + '/member/hao360login/Hao360Authorize.aspx', //好360
        ...
        popmsn: sso_consts.rootUrl + '/member/MsnLogin/MsnAuthorize.aspx', //MSN
...
+ "&nbsp;<a href='http://my.ctrip.com/Home/PassWord/VerifyUserInfo.aspx' id='sso_forgetPwd' target='_blank'>忘记密码？</a></li>"
...
+ "&nbsp;<img width='88' height='32' src='http://pic.c-ctrip.com/membercenter131211/z1.png' alt='' id='sso_imgCode'></li>"
...
+ "&nbsp;<img width='88' height='32' src='http://pic.c-ctrip.com/membercenter131211/z1.png' alt='' id='sso_imgCodePhone'></li>"
...
+ "<ul class='lg_account_list_m'><li><a href='javascript:;' id='sso_msn'><span><i class='lg_ico_msn_m' ></i></span>MSN</a></li><li><a href='javascript:;' id='sso_baidu'><span><i class='lg_ico_baidu_m' ></i></span>百度</a></li>"
+ "<li><a href='javascript:;' id='sso_renren'><span><i class='lg_ico_renren_m' ></i></span>人人网</a></li><li><a href='javascript:;' id='sso_hao360'><span><i class='lg_ico_360_m' ></i></span>360</a></li>"
...
function loginFail(data) {
    var errmsg = ['RiskInputVerifyCode', 'RiskControlRule', 'BaojieActivedRule', 'UIDExitsAfterFindRule',
                         ...
                         'NotService', 'CardNotMatch', 'NoSuchUserType', 'NoSuchSourceId', 'CardNotActive', 'CannotLogin', 'CardPleaseActive', 'ResetPasswordByPwdIsNull'],

        errmsg_st = ['请输入验证码！', '用户名或密码错误！', '激活邮件已发至您宝洁邮箱，请点击链接激活！', '用户名或密码错误！',
                         ...
                         '很抱歉，携程无法满足您的服务需求！', '卡号和公司不匹配！', '没有该类用户！', '没有指定渠道！', '卡号未开通！', '该渠道不允许登录！', '请先激活您的卡！', '您的登录密码存在风险，请点击忘记密码！'],

        errmsg_bt = ['請輸入驗證碼！', '用戶名或密碼錯誤！', '啟動郵件已發至您寶潔郵箱，請點選連結啟動！', '用戶名或密碼錯誤！',
        ...
        checkIsBlack: function (data) {
            ...
            if (data.code == 295) {
                ...
                if (data.code == 255) {
                    ...
                    jpIsBlack = "true";
                    vcodeLen = 4;
                    sso_popLogin.showVcode(true, 4);
                } else if (data.code == 256) {
                    sso_divVerifyCode.style.display = "block";
                    jpIsBlack = "true";
                    vcodeLen = 6;
                    sso_popLogin.showVcode(true, 6);
...
var sso_popLogin = {
    ...
    onbtnclick: function () {
        ...
        sso_btnSubmit.onclick = function () {
            ...
            var loginIfm = function () {
                if (/^[0-9][0-9]{7}|^[0-9][0-9]{9}$/.test(uid)) {
                    var CheckIsCtripCardcallback = 'tmp_' + parseInt(Math.random() * 1000000, 10);
                    var url = sso_Config.jsonpUrl.CheckIsCtripCard + "?CardNum=" + uid + sso_popLogin.gettmp('&', 1000) + "&jsonp=" + CheckIsCtripCardcallback;
                    window[CheckIsCtripCardcallback] = function (data) {
                        if (data.code == "T") {
                            var host = "http://ct.ctrip.com/crptravel/index.aspx?cardno=";
                            sso_popLogin.showErrors(sso_consts.sso_logintype_member, "<i></i>请进入商旅首页<a href='" + host + uid + "' target='_blank'>登录</a>查询");
                        } else {
                            if (valid_status) {
            ...
            if (valid_status) {
                if (sso_divVerifyCode.style.display != 'none') {
                    if (vcode) {
                        if (vcode === sso_consts.sso_tip_verycode || sso_trim(vcode) === '') {
                    ...
                    if (data.code === "F") {
                        ...
                        window[callbackgetuid] = function (data) {
                            if (data.code === "ssof") {
                                ...
                                window.location.href = "http://my.ctrip.com/Home/PassWord/VerifyUserInfo.aspx?uid=" + data.code;
                            }
                        }
                    }
                }
    ...
    onthridlogin: function () {
        ...
        sso_hao360.onclick = function () {
            openwin(hao360, 600, 800, 300, 300);
        };
        ...
        sso_msn.onclick = function () {
            openwin(msn, 600, 800, 300, 300);
        };
    ...
    SendSmsByLogin: function () {
        ...
        //判断是否需要验证码
        var callbackisphonecode = 'tmp_' + parseInt(Math.random() * 1000000, 10);
        var phonecodeurl = sso_Config.jsonpUrl.CheckIsDisplayPhoneCode + "?jsonp=" + callbackisphonecode;
        window[callbackisphonecode] = function (data) {
            if (data.code == "T") {
                //出验证码
                $__SSO_ID('sso_phoneCodeLi').style.display = "block";
                sso_popLogin.showPhoneVcode(true);
            } else {
                $__SSO_ID('sso_phoneCodeLi').style.display = "none";
            }
            //                loginIfm();
            $__SSO_ID('sso_phoneLoginTip').style.display = "none";
            ...
            var validForm = function (uid, vcode) {
                ...
                if ($__SSO_ID('sso_phoneCodeLi').style.display == "block" && (!vcode || vcode === sso_consts.sso_tip_verycode || sso_trim(vcode) === '')) {
                    sso_popLogin.showErrors(sso_consts.sso_logintype_phone, sso_consts.sso_error_emptyverycode);
                    return false;
                }
            ...
            if (valid_status) {
                if ($__SSO_ID('sso_phoneCodeLi').style.display == "block") {
                    var callbacksend = 'tmp_' + parseInt(Math.random() * 1000000, 10);
                    var url = sso_Config.jsonpUrl.checkVerifyCode + "t=8&username=" + uid + "&code=" + vcode + sso_popLogin.gettmp('&', 1000) + "&jsonp=" + callbacksend;
                    window[callbacksend] = function (data) {
                        if (data.code == "1") {
                            sso_popLogin.hideErrors();
                            var sendPwdUrl = sso_Config.jsonpUrl.sendDyncPwd + sso_popLogin.gettmp('&', 1000) + '&mobile=' + uid + "&vcode=" + vcode;
                            sso_popLogin.jsonpRequest(sendPwdUrl);
                        } else {
                            sso_popLogin.showErrors(sso_consts.sso_logintype_phone, sso_consts.sso_error_verycode);
                            sso_txtCodePwd.value = '';
                            sso_txtCodePwd.focus();
                            valid_status = false;
                        }
        ...
        sso_popLogin.jsonpRequest(phonecodeurl);
    },
    //倒数事件
    ...
    showPhoneVcode: function () {
        this.jsonpRequest(sso_Config.jsonpUrl.getCodePhoneImage + this.gettmp('&', 1000));
    },
